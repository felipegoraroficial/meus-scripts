with prod as (
    ...
    left join {{source('sources','suppliers')}} sp on (pd.suppliers_id = sp.suppliers_id)
    ...
    select ord.order_date, ord.order_id, cs.company_name customer, em.name employees, em.age, em.lengthofservice
    from {{source('sources','orders')}} ord
    left join {{ref('customers')}} cs on (ord.customer_id = cs.customer_id)
    left join {{ref('employees')}} em on (ord.employees_id = em.employees_id)
